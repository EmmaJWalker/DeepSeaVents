...
# get the calculations necessary for calculating the QED metrics done in parallel for each alpha:
    for (l in 1:length(alphas)){
      Sys.time()
      results<-QED.metrics(landscape=landscape, e.rate=0.1, c.rate=0.2, gamma=0, epsilon=n.patches, self.rec=1, alpha=alphas[l])
...
##################################################################################################################################
## Step 1 part 2:
# calculate the QED metrics using that data for all of the r.on and r.off combinations (could be done in parallel too:
for (l in 1:length(alphas)){
  ...
  for (j in 1:length(r.offs)){
    for (k in 1:length(r.ons)){
      ...
      QED.output<-quasi.eq(n.patches, r.ons[j], r.offs[k])
...
write.csv(all.configs, "step 1 all.configs.csv")
######################################################################################################################################
...
for (l in 1:length(alphas)){
  ...
  results<-data.frame(lambda.M, lambda.M.delta, metapop.size, n.on, pstar.configs)
  Sys.time()
  write.csv(results, file=paste0("step 2 ", n.patches, " patches config metrics alpha_", alphas[l], ".csv"))}}}
Sys.time()
################################################################################################################################
...
# calculate the QED metrics using that data for all of the r.on and r.off combinations (could be done in parallel too:
for (l in 1:length(alphas)){
  config.metrics<-read.csv(file=paste0("step 2 ", n.patches, " patches config metrics alpha_", alphas[l], ".csv"))
  lambda.M<-config.metrics$lambda.M
  lambda.M.delta<-config.metrics$lambda.M.delta
  metapop.size<-config.metrics$metapop.size
  n.on<-config.metrics$n.on
