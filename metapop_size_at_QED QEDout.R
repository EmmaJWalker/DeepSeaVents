metapop.size.at.QED<-function(landscape, e.rate, c.rate, gamma, epsilon, self.rec, alpha, r1, r2, QED){
  ...
  QED.output<-QED
  QEDdist<-QED.output[[1]]#QED distribution
  T.absorp<-QED.output[[2]]#Time to absorption
  #weighted.metapop.size<-dot(QEDdist,metapop.size[2:2^n.patches]) #this is the expected metapop size in a given configuration x the porportion of time spent in it at QED and summed (therefore the arithmentic mean)
  #should actually be the geometric mean
  weighted.metapop.size<-exp(dot(QEDdist,log(metapop.size[2:2^n.patches])))
