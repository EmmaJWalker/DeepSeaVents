...
single.batch<-function(i){
  ...
  source("QED_metrics_func_for_large_N_IDsToConfigs.r") #QED.metrics()
  source("dewoody_func.r") #dewoody()
  source("Pstar_func.r") #pstar.function()
  source("reduced_quasi_eq_sparse_func2.r") #quasi.eq()
  source("IDs_to_configs_func2.r")
  source("SRLMODE_func.r") #SRLMODE_func      SRLM.ODE()
  ...
  r.offs<-c(1/100000,1/1000,1/10)
  r.ons<-c(1/100,1/10,1)
  alphas<-c(1/100, 1/5, 1/20)# global, stepping stone 1/4 stepping stone, c(10,1/10,1/100) #avg. disp = 1/10 interpatch distance, the interpatch distance and global disp
  n.patches<-20
  ...
  for (j in 1:length(r.offs)){
    for (k in 1:length(r.ons)){
      for (l in 1:length(alphas)){
        ...
        c.rate<-1  #can pick just any number to start since we will just rescale this
        ...
        metrics<-QED.metrics(s.size=10000, landscape=landscape, e.rate=e.rate, c.rate=c.rate, gamma=gamma, epsilon=epsilon, self.rec=self.rec, alpha=alpha, r.on=r.on, r.off=r.off)
        #takes about a minute
        delta<-metrics[[2]]/Initial.Lm
        mag.delta<-round(log10(1/delta)) # need to ensure e.rate and c.rate are less than one
        #this is always the case if just scaling e.rate by delta since delta will be less than one
        #if want to scale c,rate by delta rather than the e.rate, can just ensure this byrescaling both 
        #so they are an order of magnitude below 1:
        e.rate<-delta#1/(10*mag.delta)#delta ********CANNOT BE GREATER THAN 1
        c.rate<-1#1/(10*mag.delta)#1 ***********CANNOT BE GREATER THAN 1
        metrics<-QED.metrics(s.size=10000, landscape=landscape, e.rate=e.rate, c.rate=c.rate, gamma=gamma, epsilon=epsilon, self.rec=self.rec, alpha=alpha, r.on=r.on, r.off=r.off)
        #takes about a minute
        exp.lm<-metrics[[1]]
       ...
       #all.configs<-all.configs[-1,] #all configs excluding the absorbing state (which we won't start in) ***no longer includes absobring state when subsampling
        pstar.configs<-metrics[[8]]
        #pstar.configs<-pstar.configs[-1,] #all configs excluding the absorbing state (which we won't start in) ***likewise
        ...
        #x<-sample(x=(1:nrow(all.configs)), size=1, prob=QED) #to choose a config to start with the probability of being in it at QED ***no longer this simple since using reduced CTMC
        ...
        n.on.sample <- sample(n.on.vec, 1, prob=QED.dist) #sub sample how many patches could 
        #possibly be on with the probability that number of patches in on at QED
        prior.configs<-cumsum(z[1:(n.on.sample)]) #calculate number of preceding configurations with lesser numbers of patches on
        x.w.n.on<-sample(1:z[n.on.sample], 1) #randomly pick a config with the given number of patches on, since all have equal probability at QED
        x<-prior.configs[n.on.sample-1]+x.w.n.on+1 #config ID is the sum of the prior configs w fewer patches on plus which config chosen with the number of patches on
        #############################
        
        initial.config<-IDs.to.configs(x=x, n.patches=n.patches)
        #initial.config<-as.matrix(as(initial.config, "sparseMatrix")) #####******
        landscape.config<-landscape[initial.config==1] #construct landscape in the initial config
        initial.p<-pstar.function(landscape.config, alpha, e.rate, c.rate, self.rec, gamma, epsilon, iterations=1000)
        ...
        simu.data<-output[[1]]
        simu.data.w.end.t<-simu.data
        simu.data<-simu.data[-nrow(simu.data),]
        ...
        write.csv(simu.data.w.end.t, paste0(n.patches, " patch Simudata Combined CTMC and SRLM simulation sample lmQED", Initial.Lm, " ron", 
                                    r.on, " roff", r.off, " a", alpha,"d", delta, "_", it.no, ".csv"))
        write.csv(configs.data, paste0(n.patches, " patch configdata Combined CTMC and SRLM simulation sample lmQED", Initial.Lm, " ron", 
                                       r.on, " roff", r.off, " a", alpha,"d", delta, "_", it.no, ".csv"))
        write.csv(summary.data, paste0(n.patches, " patch summary data lmQED", Initial.Lm, " ron",                                
                                       r.on, " roff", r.off, " a", alpha,"d", delta, "_", it.no, ".csv"))
        configs.n.pstars<-data.frame(as.matrix(all.configs), pstar.configs)
        write.csv(configs.n.pstars, paste0(n.patches, " patch configs.n.pstars", Initial.Lm, " ron",                                
                                           r.on, " roff", r.off, " a", alpha,"d", delta, "_", it.no, ".csv"))
        gc()#clear the memory
        }
      }
