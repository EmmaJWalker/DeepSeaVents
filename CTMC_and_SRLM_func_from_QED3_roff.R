CTMC.SRLM<-function(total.t, landscape, e.rate, c.rate, alpha, gamma, epsilon, 
  ...
  ###############################################
    ...
    } else if (Norm(x1)<=(1e-10)){ #if the last change in occupancy was less than 1e-4
  ...
  ######################################################################################################## ADDED JULY 2023
  if(sum(config)==0){
    configs.data<-rbind(configs.data, c(end.t, config)) #to include when hit absorbing state in the configs.data
    if (Norm(x1)<=(1e-10)){ #if the last change in occupancy was less than 1e-10
      #the SRLM reached equilibrium before endt, ending the solver early to save on computation
      #so we shall append the output with that same equilibrium occupancy at endt
      sim.data<-rbind(sim.data, c(end.t, output[length(sim.data[,1]),-1])) #to include size of metapop when the habitat was lost
    }
  }
  ########################################################################################################
