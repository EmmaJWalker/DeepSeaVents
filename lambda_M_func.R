...
##################################################################################################################
get.lambda.M<-function(landscape, alpha, gamma, epsilon, self.rec, e.rate, c.rate){
...
#################################################################################################################################
#CHECK
#lambda.M<-get.lambda.M(landscape=landscape, alpha=0.1, e.rate=0.1, c.rate=4, self.rec=1, gamma=0, epsilon=n.patches)
#lambda.M
#20/lambda.M
...
#e.rate<-delta
#c.rate<-1
#lambda.M<-get.lambda.M(landscape=landscape, alpha=0.1, e.rate=e.rate, c.rate=c.rate, self.rec=1, gamma=0, epsilon=n.patches)
#lambda.M
