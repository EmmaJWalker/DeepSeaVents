...
QED.metrics<-function(landscape, e.rate, c.rate, gamma, epsilon, self.rec, alpha, r.on, r.off){
  ...
  for(i in 1:nrow(all.configs)){
    if (i==1){
      ...
      lambda.M[i]<-get.lambda.M(r.off, landscape.config, alpha, gamma, epsilon, self.rec, e.rate, c.rate)/r.off
      lambda.M.delta[i]<-get.lambda.M(r.off, landscape.config, alpha, gamma, epsilon, self.rec, e.rate, c.rate)/delta
      if (lambda.M.delta[i] < delta){
        ...
        pstar.configs[i,all.configs[i,]!=0]<-pstar.function(r.off, landscape.config, alpha, e.rate, c.rate, self.rec, gamma, epsilon, iterations=1000)
        }
    }
    }
