...
source("QED_metrics_func_roff.r")
...
for (j in 1:length(r.offs)){
  for (k in 1:length(r.ons)){
    for (l in 1:length(alphas)){
      ...
      x<-QED.metrics(landscape=landscape, e.rate=e.rate, c.rate=1, gamma=gamma, epsilon=epsilon, self.rec=self.rec, alpha=alphas[l],r.on=r.ons[k], r.off=r.offs[j])
      ...
      write.csv(new.configs.n.pstars, paste0("new.configs.n.pstars",Initial.Lm,
                                             " ron",r.ons[k]," roff",r.offs[j]," a",alphas[l],"roff.csv"))
    }
      ...
      plot.data$exp.n.on[(plot.data$r.off==r.offs[j]&plot.data$r.on==r.ons[k])]<-rep(x[[5]],r) #########****************8
...
#write.csv(plot.data, "plot_data_600reps2 roff.csv")
######################################################################################################

# reading in this data with these new values added
######################################################################################################
plot.data<-read_csv("plot_data_600reps2 roff.csv")
#####################################################################################################
...
write.csv(p, "Time to absorption table roff.csv")
#######################################################################################################
...
table2
write.csv(table2, "table2 roff.csv")
########################################################################################################
...
guides(fill=guide_legend("Log Rate of Disturbance", override.aes = list(fill=pal, alpha=0.25, colour=pal)), 
       ...
       colour="none")
ggsave2("Expected versus Average Simulated Metapopulation Sizes Across Disturbance Regimes Global Dispersal only roff.jpg", 
...
ggplot(p.data) + 
  ...
  guides(fill=guide_legend("Log Rate of Disturbance", override.aes = list(fill=pal, alpha=0.25, colour=pal)), 
         linetype=guide_legend("Estimate", override.aes = list(linetype=1, alpha=1, colour=c("#016FB9", "black"))),
         colour="none")
ggsave2("Expected versus Average Simulated Metapopulation Sizes Across Disturbance Regimes Stepping Stone only roff.jpg", 
        height=4.5, width=10, units="in", dpi=800)
########################################################################################################
...
ggplot(p.data) + 
  ...
  guides(fill=guide_legend("Log Rate of Disturbance", override.aes = list(fill=pal, alpha=0.25, colour=pal)), 
         linetype=guide_legend("Estimate", override.aes = list(linetype=1, alpha=1, colour=c("#016FB9", "black"))),
         colour="none")
ggsave2("Expected versus Average Simulated Metapopulation Sizes Across Disturbance Regimes 1 10th Stepping Stone only roff.jpg", 
        height=4.5, width=10, units="in", dpi=800)
################################################################################################
