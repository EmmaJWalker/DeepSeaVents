#a version of the CTMC.SRLM function that simulates over a limited number of 
#transitions rather than timesteps

CTMC.SRLM<-function(total.trans, landscape, e.rate, c.rate, alpha, gamma, epsilon, 
                    self.rec, r.on, r.off){
  ...
  config<-rbinom(n.patches, 1, 0.5) #start in random configuration
  #config<-rep(1,n.patches) #start with all on
  ...
  transition.limit<-total.trans #setting a limit to the number of transitions we sample
  sim.data<-NA
  while (sum(config) > 0 & trans < transition.limit) { 
    ...
    if (sum(tail(output[,-1])) <= 0){ break } #stop the simulation if the metapopulation goes extinct.
...
#
#test<-CTMC.SRLM(total.trans<-2000, landscape=landscape, e.rate<-0.1, c.rate<-0.2, alpha<-10, gamma<-0, 
