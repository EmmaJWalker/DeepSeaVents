...
#################################################################################################################
dewoody.pstar<-function(landscape, alpha, e.rate, c.rate, self.rec, gamma, epsilon, iterations, r.on, r.off){
  n.patches<-length(landscape$patch.ID)
  x.coord<-landscape$x.coord
  y.coord<-landscape$y.coord
  AREA<-landscape$areas
  dewoody.delta<-(e.rate+r.off)/(c.rate*r.on/(r.on+r.off)) #modified as in Dewoody et al. 2005
  ...
  for (i in 1:n.patches){ #for each patch
    for (j in 1:n.patches){ #to each other patch (including itself)
      M[i,j]<-AREA[j]*DISPKERNEL[i,j] #modified as in Dewoody et al. 2005
    }
  }
  ...
  for (t in 1:(iterations-1)){ #iterate the following for a given number of iterations
    ...
    f<-(g/(g+dewoody.delta)) #SRLM f function from Ovaskainen and Hanski, 2001, modifed based on Dewoody et al. 2005
