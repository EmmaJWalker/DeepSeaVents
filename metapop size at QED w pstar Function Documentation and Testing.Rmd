...
###############################################################################
metapop.size.at.QED<-function(landscape, e.rate, c.rate, gamma, epsilon, self.rec, alpha, r.on, r.off){
  ...
  for (i in 0:((2^n.patches)-1)){
    ...
    if (i==0){
      ...
      #below the extinction threshold, then the metapopultion is extinct, set size = 0
        ...
        metapop.size[i+1]<-pstar.function(landscape.config, alpha, delta, iterations=1000)
        }
    }
  }
