...
QED.metrics<-function(landscape, e.rate, c.rate, gamma, epsilon, self.rec, alpha, r.on, r.off){
  ...
  sorted.QED<-sort(QED, decreasing=F)
  med.i<-which(cumsum(sorted.QED)>0.5)
  med.QED<-sorted.QED[med.i[1]]
  QEDndata<-data.frame(QED, n.on.in.each.config[2:2^n.patches])
  med.exp.n.QED<-QEDndata$n.on.in.each.config[QED==med.QED]
  
  #avg. time to absorption from QED
  QED.T.absorp<-exp(dot(QED, log(as.vector(T.absorp))))
  
  all.configs<-as.matrix(all.configs)
  pstar.configs<-as.matrix(pstar.configs)
  
  return(list(lm.QED.delta, lm.QED, geom.pstar.QED, ar.pstar.QED, exp.n.QED, QED, all.configs, pstar.configs, med.exp.n.QED, QED.T.absorp))
}
